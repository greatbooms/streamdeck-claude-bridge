...
from bridge.models import question_from_hook, codex_permission_from_hook, normalize_session_id
...
def make_codex_permission_handler(store, hub):
    async def handler(request):
        body = await request.json()
        q = codex_permission_from_hook(body)
        if q is None:
            return web.json_response({"ok": False, "reason": "no session/permission"})
        store.add(q)
        await hub.broadcast({"type": "question_added", "question": q.to_dict()})
        return web.json_response({"ok": True})
    return handler
